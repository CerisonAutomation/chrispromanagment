// =============================================================================
// CANONICAL PUCK MIGRATION SYSTEM
// Mirror of puck-main/packages/core/lib/migrate.ts
...
import {walkAppState, walkTree} from "./utils/walk-transform";
...
export function needsMigration(data: Data): boolean {
  // Check if root has props or not
  const hasRootProps = "props" in data.root && typeof data.root.props === "object";
  
  // Check if zones exist (legacy structure)
  const hasZones = "zones" in data && Object.keys(data.zones || {}).length > 0;
  
  return !hasRootProps || hasZones;
}
