...
return false;
    ...
    if (filter.canSuggest !== null && filter.canSuggest !== undefined && b.ai.canSuggest !== filter.canSuggest) {
return false;
}
    if (filter.useCase && !b.ai.useCases.includes(filter.useCase)) {
return false;
}
    return true;
  });
...
export function validateBlock(typeId: string, content: unknown) {
  const def = getBlock(typeId);
  if (!def) {
return { success: false, error: `Unknown block type: ${typeId}` };
}
  return def.validate(content);
}
