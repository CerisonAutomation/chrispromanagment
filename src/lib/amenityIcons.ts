...
import {
  ...
  Bath, Bed, DoorOpen, Flame, Lock, Waves, Sun, Shirt,
  Dumbbell, Baby, Dog, Cat, Cigarette, CigaretteOff,
  Accessibility, Refrigerator, Microwave, ChefHat, UtensilsCrossed,
  Laptop, Briefcase, BookOpen, Gamepad2, Monitor,
  Home, TreePine, Umbrella,
  Users, Shield, Bell, Calendar,
  Thermometer, Fan, Droplets, Sparkles, Zap,
  Heart, CheckCircle,
  ArrowUp,
  Moon, Globe,
  Package,
  User, Network, Smartphone, Lamp, Heater,
} from "lucide-react";
...
export const getAmenityIcon = (amenityKey) => {
  if (!amenityKey) {
return { icon: CheckCircle, label: "Amenity", category: "other" };
}
  ...
  if (AMENITY_ICONS[normalizedKey]) {
return AMENITY_ICONS[normalizedKey];
}
  ...
  if (mappedKey && AMENITY_ICONS[mappedKey]) {
return AMENITY_ICONS[mappedKey];
}
