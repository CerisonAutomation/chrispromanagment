...
function zodFieldToPuckField(name: string, schema: z.ZodTypeAny): PuckField {
  ...
  if (typeName === 'ZodObject') {
    // For nested objects, flatten to text for simplicity
    // Complex nested objects should be handled as arrays
    return {
      type: 'textarea',
      label: formatLabel(name),
...
export function generateStubRender(blockType: string): (props: Record<string, unknown>) => React.ReactNode {
  return function StubRender(props: Record<string, unknown>): React.ReactNode {
    // Create a simple placeholder that shows block info in the editor
...
/**
 * Validates props against a Zod schema, returning sanitized props.
 */
...
/**
 * Gets the default values from a Zod schema.
 */
export function getSchemaDefaults<T>(schema: z.ZodType<T>): Partial<T> {
  try {
    // Parse with undefined to get defaults
